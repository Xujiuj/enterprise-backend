...
(900219, 'Source(A)数据导入', 900130, 15, '#', '', '', 1, 0, 'F', '1', '0', 'enterprise:sourceA:import', '#', 103, 1, sysdate(), 'Source(A)多工作簿数据导入权限'),
(900220, 'Source(A)数据校验', 900130, 16, '#', '', '', 1, 0, 'F', '1', '0', 'enterprise:sourceA:validate', '#', 103, 1, sysdate(), 'Source(A)已导入数据校验权限'),
(900221, '活动数据修改', 900130, 17, '#', '', '', 1, 0, 'F', '1', '0', 'enterprise:activityDataRaw:edit', '#', 103, 1, sysdate(), '活动数据修改权限'),
(900222, '活动数据删除', 900130, 18, '#', '', '', 1, 0, 'F', '1', '0', 'enterprise:activityDataRaw:remove', '#', 103, 1, sysdate(), '活动数据删除权限');
...
-- Delete menus that are no longer part of the current enterprise portal contract.
-- Hidden rows still appear in menu management, so stale M/C rows and stale buttons are removed instead of hidden.
delete srm
from sys_role_menu srm
join sys_menu sm on sm.menu_id = srm.menu_id
where sm.menu_type in ('M', 'C')
  and sm.menu_id not in (
    900100, 900102,
